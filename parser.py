...
def p_if_statement(p):
    '''if_statement : IF expression COLON block elif_statements else_statement
                    | IF expression COLON block elif_statements
...
def p_for_statement(p):
    ...
    global indent_level
    indent_level += 1
    p[0] = f"{indent()}for (let {p[2]} = 0; {p[2]} < {p[6]}; {p[2]}++) {{\n{p[9]}{indent()}}}\n"
    indent_level -= 1
...
def p_assignment_statement(p):
    ...
    indent_level += 1
    if p[2] == '=':
        p[0] = f"{indent()}let {p[1]} = {p[3]};\n"
    else:
        p[0] = f"{indent()}{p[1]} {p[2]} {p[3]};\n"
